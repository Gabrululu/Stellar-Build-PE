...
export async function getVotes(): Promise<Record<string, number>> {
  try {
    ...
    // scValToNative convierte ScvMap a un JavaScript Map<string, number>
    const nativeMap = scValToNative(retval) as Map<string, number>;
...
export async function checkHasVoted(walletAddress: string): Promise<boolean> {
  try {
    const tx = new TransactionBuilder(DUMMY_ACCOUNT, {
      ...
      .addOperation(
        contract.call('has_voted', new Address(walletAddress).toScVal())
      )
      .setTimeout(30)
      .build();
